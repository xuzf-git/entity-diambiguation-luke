...
def load_person_names(input_file):
    ...
    Args:
        input_file (string): data/entity_disambiguation/persons.txt
...
def load_documents(csv_path, conll_path, person_names):
    ...
    Args:
        csv_path (string): csv_format文件的路径
        conll_path (string): conll_format文件的路径
        person_names (list[string]): 人名列表

    Returns:
        list[Document]: 
        [
            {
                id : '304 Malta) 304 Malta)',
                words : ['Malta', 'police', 'seize', ... ,'1996-08-24', '', 'Police', 'in', ...],
                mentions : 
                [
                    {
                        text : 'Malta',
                        title : 'Malta',
                        start : 0, end : 1, span : (0,1),
                        candidates : [
                            {title : 'Malta', prior_prob : 0.676}
                            {title : 'Malta national football team', prior_prob : 0.06},
                            ...
                            ]
                    },
                    ...
...
def load_mentions_from_csv_file(path, person_names):
    ...
    for _, mentions in mention_data.items():    # for each doc
        for mention in mentions:    # for each mention of doc
...
# if __name__=="__main__":
    # dataset = EntityDisambiguationDataset("/data_1/xuzf/workspace/opensource/OpenEL/luke/data/entity_disambiguation")