...
class TyDiExample(object):
  ...
  """
    ...
    def __init__(
        ...
        contexts: str,
        plaintext: str,
        context_to_plaintext_offset: List[int],
        answer: Answer = None,
    ...
    for a in positive_annotations:
        if a["minimal_answer"]:
            ...
            return (
                ...
                idx,
                (
                    global_to_local_offset(json_dict, idx, start_byte_offset),
...
def global_to_local_offset(json_dict, candidate_idx, byte_index):
    ...
    global_start = json_dict["passage_answer_candidates"][candidate_idx]["plaintext_start_byte"]
...
def candidates_iter(json_dict: Dict):
...
def make_tydi_answer(contexts, answer):
  ...
  """
    ...
    if answer["candidate_id"] == -1 or start >= byte_len(contexts) or end > byte_len(contexts):
    ...
    return Answer(answer_type, text=byte_slice(contexts, start, end), offset=start)
