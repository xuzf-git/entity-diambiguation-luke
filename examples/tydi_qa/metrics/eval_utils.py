...
class Span(object):
  ...
  """
    ...
    def __init__(self, start_byte_offset, end_byte_offset):
        ...
        if (start_byte_offset < 0 and end_byte_offset >= 0) or (start_byte_offset >= 0 and end_byte_offset < 0):
            raise ValueError("Inconsistent Null Spans (Byte).")

        if start_byte_offset >= 0 and end_byte_offset >= 0 and start_byte_offset > end_byte_offset:
...
def compute_partial_match_scores(gold_span, pred_span):
  ...
  """
    ...
    if gold_span.is_null_span():
        raise ValueError("Null gold span should not be passed for F1 computation.")
    if pred_span.is_null_span():
        raise ValueError("Null prediction span should not be passed for F1 computation.")
    ...
    if (gold_span.end_byte_offset <= pred_span.start_byte_offset) or (
        ...
        in_both = min(gold_span.end_byte_offset, pred_span.end_byte_offset) - max(
            gold_span.start_byte_offset, pred_span.start_byte_offset
        )
        assert in_both > 0
        ...
        if gold_span.start_byte_offset <= pred_span.start_byte_offset:
            ...
            only_in_gold += max(0, gold_span.end_byte_offset - pred_span.end_byte_offset)
            only_in_pred = max(pred_span.end_byte_offset - gold_span.end_byte_offset, 0)
        # if pred span starts earlier than gold span.
        else:
            only_in_pred = gold_span.start_byte_offset - pred_span.start_byte_offset
            only_in_pred += max(0, pred_span.end_byte_offset - gold_span.end_byte_offset)
            only_in_gold = max(gold_span.end_byte_offset - pred_span.end_byte_offset, 0)
...
def gold_has_minimal_answer(gold_label_list, minimal_non_null_threshold):
    ...
    gold_has_answer = (
        ...
        and sum(
            [
                ((not label.minimal_answer_span.is_null_span()) or (label.yes_no_answer != "none"))
    ...
    for single_prediction in predictions:
        ...
        if "yes_no_answer" in single_prediction:
            ...
            if yes_no_answer != "none" and not minimal_span.is_null_span():
                raise ValueError("yes/no prediction and minimal answers cannot coexist.")
...
def read_annotation_from_file(input_file):
    ...
    for line in input_file:
        ...
        for annotation in json_example["annotations"]:
            ...
            minimal_span = Span(minimal_span_dict["plaintext_start_byte"], minimal_span_dict["plaintext_end_byte"])
